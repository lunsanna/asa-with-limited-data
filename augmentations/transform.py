...
def pitch_shift(data_args: Dict[str, Any],
    ...
    do_gaussian: bool = transform_args.get("gaussian")
    sigma: int = transform_args.get("sigma")
    range: int = transform_args.get("range")
    sampling_rate = data_args.get("target_feature_extractor_sampling_rate")
    assert sampling_rate and (do_gaussian is not None) and (sigma if do_gaussian else range)

    def random_pitch_shift():
        return np.random.normal(0, sigma) if do_gaussian else np.random.uniform(-range, range)
...
def additive_noise(data_args: Dict[str, Any], 
    ...
    noise_paths = glob.glob(f"{noise_dir}/*")
